...
# The installed unit was generated by install.sh at install time and does not
# track later template changes. Sync the one directive that matters for
# survival: Restart=always (a clean SIGTERM from site kill sweeps exits 0,
# which Restart=on-failure never restarts).
UNIT=/etc/systemd/system/sgpu-collector.service
if [ -f "$UNIT" ] && ! grep -q '^Restart=always' "$UNIT"; then
    echo "   syncing Restart=always into $UNIT"
