...
SLURM 클러스터 GPU 운영을 위한 실시간 모니터링 도구입니다. 터미널 TUI,
collector 데몬, compute node push 에이전트, 사용량/낭비 집계, Slack 알림까지
한 번에 제공합니다.
...
`sgpu`는 `sinfo`, `squeue`, 선택적으로 `sacct`를 쓸 수 있고 GPU 노드로
passwordless SSH가 가능한 Slurm 로그인/마스터 노드에서 운영하는 것을 기준으로
합니다.

- `sgpu-collector`가 마스터에서 계속 실행되며 병합된 클러스터 상태를
  `/tmp/slurm-gpu-tui/data.json`에 기록합니다.
- `sgpu-agent`는 각 GPU 노드에서 `nvidia-smi` 데이터를 공유 파일시스템으로
  push할 수 있습니다. push 조건이 안 맞으면 collector가 자동으로 SSH-pull로
  폴백합니다.
- TUI는 collector 출력 파일을 먼저 읽으므로 큰 클러스터에서도 `sgpu` 실행이
  즉시 열립니다.
- Slack 알림은 collector 스냅샷과 `~/.sgpu/state`의 영속 상태를 기준으로
  발생하므로 재시작해도 오래된 지속 알림을 다시 쏘지 않습니다.
- 설치 후 또는 데이터/알림 이상 시 첫 확인 명령은 `sgpu doctor`입니다.
...
**Push 모드 (권장):** 각 GPU 노드에 상주하는 작은 `sgpu-agent`가 자기 통계를 공유 파일시스템 디렉토리에 몇 초마다 기록합니다. 마스터의 collector는 이 파일을 로컬로 읽기만 하므로 핫패스에 SSH가 없고, sshd가 불안정하거나 노드가 바빠도 수집이 멈추지 않습니다.
...
Collector는 `/tmp/slurm-gpu-tui/metrics.prom`도 기록합니다 (Prometheus
textfile 형식: GPU util/memory/temp/power, 할당, free/rogue/waste 집계, idle
초, 노드 헬스). node_exporter의 textfile collector에 붙이고 포함된 Grafana
대시보드를 import하면 됩니다.

**→ Grafana 설정과 dashboard import: [docs/GRAFANA.md](docs/GRAFANA.md)**
