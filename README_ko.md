...
- Slack 알림(노드 다운/복구, GPU 헬스, 낭비/rogue)을 일별 스레드로
...
sgpu doctor          # 자가진단: 데이터, 에이전트, slurm, sacct, webhook
...
설정은 `~/.sgpu/webhook.json`(핫 리로드); 설치 스크립트가 세팅하고
...
| `SLURM_GPU_TUI_WEBHOOK_URL` | (없음) | Slack webhook URL(전체 설정: `~/.sgpu/webhook.json`) |
| `SLURM_GPU_TUI_SLACK_BOT_TOKEN` | (없음) | 일별 스레드 모드용 Slack 봇 토큰 |
| `SLURM_GPU_TUI_WEBHOOK_DEBOUNCE_SEC` | `1800` | 반복 알림 최소 간격 |
| `SLURM_GPU_TUI_WEBHOOK_NAG_SEC` | `21600` | 지속 상태 재알림 간격 |
